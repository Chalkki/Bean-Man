...
public class YellowGhostMovement : MonoBehaviour
{
    private float speed;
    GameObject player;
    ...
    void Update()
    {
        ...
        {
            // the yellow ghost will chase the position 4 blocks ahead of the player

            targetPos = player.transform.position + PosOffset();
            if (speed == 0)
            {
                speed = player.GetComponent<PlayerMovement>().GetPlayerSpeed() * 0.85f;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            if (frightenMode)
            {
                gameController.GetComponent<Controller>().GhostRespawn(this.gameObject, 2);
