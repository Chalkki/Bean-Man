...
public class BlueGhostMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isCaged)
        {
            ...
            {
                transform.position = new Vector3(transform.position.x, transform.position.y + 3, transform.position.y);
                isCaged = false;
            }
            return;
        }
        ...
        else if(redGhost != null)
        {
            // the blue ghost will target at the reverse of the difference between the red ghost and the position at 2 blocks ahead of the player
            targetPos = finalPos();
            if (speed == 0)
            {
                speed = player.GetComponent<PlayerMovement>().GetPlayerSpeed() * 0.85f;
    ...
    // set the offset to be 4 blocks ahead of the player
    Vector3 finalPos()
    {
        Vector3 offset = Vector3.zero;
        if (player.transform.eulerAngles.z == 0)
        {
            // if the player is facing either left or right
        ...
        // calcuate the difference between the offseted position and the red ghost's position
        Vector3 offsetedPos = player.transform.position + offset;
        Vector3 difference = redGhost.transform.position - offsetedPos;

        // reverse the difference vector to be final position
        Vector3 finalPos = -difference;
        return finalPos;
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            if (frightenMode)
            {
                gameController.GetComponent<Controller>().GhostRespawn(this.gameObject, 1);
